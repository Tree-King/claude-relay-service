...
- [Claude Code 版本對齊說明](docs/claude-code-versioning.md)：說明統一版本校驗的來源、判斷條件與排查步驟，避免客戶端與服務端版本不一致導致被攔截。
- [對話紀錄寫入策略](docs/conversation-logging.md)：整理對話保存的範圍、字段與過期策略，包含串流錯誤回補與文本截斷細節。
